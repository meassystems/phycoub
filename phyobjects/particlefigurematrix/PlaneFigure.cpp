...
PlaneFigure::PlaneFigure(const Vector& directionLine, const Vector& directionPlane, const int& numInLine, const int& numLineInPlane, const Vector& coordinate, const Vector& speed, const double& m, const double& z, BorderCondition* borderCondition) {
	allParticles_.reserve(numInLine * numLineInPlane);
	for(int i = 0; i < numLineInPlane; ++i) {
		for(int j = 0; j < numInLine; ++j) {
			allParticles_.push_back(new Particle(Vector(coordinate + directionLine * i + directionPlane * j), speed, m, z, borderCondition));
