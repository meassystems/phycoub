...
NOCoub::NOCoub() {
	...
	feelField_.push_back(FeelField(&createField_.back(), &azotInterworking, "LD NN Feel"));
	/*for(int i = 0; i < 100; i++) {
		azot_.push_back(new Particle(Vector((rand()/(double)RAND_MAX)*borders_.x_, (rand()/(double)RAND_MAX)*borders_.y_,
				(rand()/(double)RAND_MAX)*borders_.z_), Vector(.0, .0, .0), mN_, z_, &cyclicBorder_));
	}*/
	azot_.push_back(new Particle(Vector(0.5*borders_.x_, 0.5*borders_.y_,
					0.45*borders_.z_), Vector(.0, .0, .0), mN_, z_, &cyclicBorder_));
	...
	feelField_.push_back(FeelField(&createField_.back(), &oxygenInterworking, "LD OO Feel"));
	/*for(int i = 0; i < 100; i++) {
		oxygen_.push_back(new Particle(Vector((rand()/(double)RAND_MAX)*borders_.x_, (rand()/(double)RAND_MAX)*borders_.y_,
				(rand()/(double)RAND_MAX)*borders_.z_), Vector(.0, .0, .0), mO_, z_, &cyclicBorder_));
	}*/
	oxygen_.push_back(new Particle(Vector(0.5*borders_.x_, 0.5*borders_.y_,
					0.55*borders_.z_), Vector(.0, .0, .0), mO_, z_, &cyclicBorder_));
	...
	feelField_.push_back(FeelField(&createField_.back(), &NOInterworking, "LD OO Feel"));
	createField_.back().addGroupParticle(azot_);
	feelField_.back().addGroupParticle(oxygen_);
	// ON
	createField_.push_back(CreateField(&NOField_, &cyclicBoundedFieldNO_, "LD NO Field"));
