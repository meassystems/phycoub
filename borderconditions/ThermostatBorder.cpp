...
ThermostatBorder::ThermostatBorder(Vector* borders, double* kB, double* temp): BorderCondition(borders), kB_(kB), temperature_(temp) {}
...
void ThermostatBorder::psyMove(const Vector& move, Particle& particle) {
	...
	for(int i = 0; i < 3; ++i) {
		...
		if(particle.coordinate_[i] < 0) {
			...
			fl = true;
			particle.coordinate_[i] = 2 * (*borders_)[i] - particle.coordinate_[i];
			particle.speed_[i] = -particle.speed_[i];
		}
	}
	if(fl) {
